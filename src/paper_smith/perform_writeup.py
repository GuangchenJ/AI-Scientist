...
If duplicated, identify the best location for the section header and remove any other."""
    ...
    for i in range(num_error_corrections):
        ...
        check_output = os.popen(
            f"chktex {writeup_file} -q -n2 -n24 -n13 -n1").read()
        if check_output:
...
- Results or insights in the `sources/source_latex.tex` that have not yet need included
...
refinement_prompt = (
    """Great job! Now criticize and refine only the {section} that you just wrote in `latex/template.tex`.
...
# PERFORM WRITEUP
def perform_writeup(
    project_name, folder_name, coder
):
    # CURRENTLY ASSUMES LATEX
    abstract_prompt = f"""We've provided the `latex/template.tex`, `sources/source_latex.tex` and `sources/source_reference.tex` file to the project. We will be rewrite the context in `latex/template.tex` according `sources/source_latex.tex` in section by section.

Please read the article in `sources/source_latex.tex` file thoroughly to understand its content fully. Once you have understood the article, please rewrite in the "Title" and "Abstract" sections of the writeup:
...
"""
    ...
    for section in [
        ...
        section_prompt = f"""Please fill in the {section} of the writeup in `latex/template.tex` refer to the contents of `sources/source_latex.tex`. Some tips are provided below:
{per_section_tips[section]}

Be sure to use \\cite or \\citet where relevant, referring to the works provided in the file.
Do not cite anything that is not already in `references.bib` or `sources/source_reference.bib`. Do not add any new entries to this.

Keep the experimental results (figures and tables) only in the Results section, and make sure that any captions are filled in.
In this pass, do not reference anything in later sections of the paper.

Please note that the contents of \\subsection and \\subsubsection also belong to the corresponding \\section. Ensure the accuracy of the equation and DO NOT CHANGE ANY MATHEMATICAL EXPRESSIONS IN THIS PAPER.
...
"""
    ...
    related_work_section_prompt = f"""Please refer to the Related Work in `sources/source_latex.tex` and fill in the Related Work of the writeup in `latex/template.tex`. Some tips are provided below:

{per_section_tips["Related Work"]}

For this section, very briefly sketch out the structure of the section, refer to the papers cited in `sources/source_latex.tex`, and clearly indicate what papers you intend to include to some extent.
...
Be sure to first name the file and use *SEARCH/REPLACE* blocks to perform these edits.
"""
    coder_out = coder.run(related_work_section_prompt)

    related_work_refinement_prompt = (
        """Great job! Now criticize and refine only the {section} that you just wrote in `latex/template.tex`.
Make this complete in this pass, do not leave any placeholders.

For the literature that has appeared in `sources/source_latex.tex`, PLEASE ENSURE THE ACCURACY OF ITS DESCRIPTION AND AVOID INCLUDING ANY RESULTS OR INSIGHTS THAT ARE NOT DESCRIBED.
...
"""
    ...
    coder_out = coder.run(
        related_work_refinement_prompt.format(section="Related Work")
        .replace(r"{{", "{")
        .replace(r"}}", "}")
    )
...
First, re-think the Title if necessary. Keep this concise and descriptive of the paper's concept, but try by creative with it."""
    ...
    generate_latex(coder, folder_name, f"{folder_name}/{project_name}.pdf")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Perform writeup for a project")
    ...
    io = InputOutput(yes=True, chat_history_file=f"{
                     folder_name}/{idea_name}_aider.txt")
